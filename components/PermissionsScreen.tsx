...
// Arrow Icon for Go Ahead button
...
const PermissionsScreen: React.FC<PermissionsScreenProps> = ({
  ...
  // Initial permission check with auto-navigation
  useEffect(() => {
    const initialCheck = async () => {
      const permissions = await checkPermissions();
      const allGranted = permissions.notification && permissions.overlay && permissions.usage;

      setIsLoading(false);

      // Auto-navigate to home if all permissions are already granted
      if (allGranted && !hasAutoNavigated.current) {
  ...
  useEffect(() => {
    const handleAppStateChange = async (nextAppState: AppStateStatus) => {
      if (nextAppState === 'active') {
        ...
        // Auto-navigate if all permissions became granted after returning from settings
        if (allGranted && !hasAutoNavigated.current) {
          hasAutoNavigated.current = true;
          onAllPermissionsGranted();
        }
  ...
  const allPermissionsGranted = notificationPermission && overlayPermission && usagePermission;
  ...
  return (
    <View style={styles.container}>
      <View style={styles.content}>
        ...
        {/* Title */}
        <Text style={styles.title}>Set Permissions</Text>
        ...
        <View style={styles.permissionsContainer}>
          ...
          {/* Overlay Permission */}
          <View style={styles.permissionRow}>
            <Text style={styles.permissionText}>Allow Screen Overlay</Text>
            <ToggleSwitch
              isEnabled={overlayPermission}
          ...
          <View style={styles.permissionRow}>
            <Text style={styles.permissionText}>Allow Usage Access</Text>
            <ToggleSwitch
              isEnabled={usagePermission}
              onToggle={handleUsageToggle}
            />
          </View>
        </View>

        {/* Go Ahead Button - shown when all permissions granted */}
        {allPermissionsGranted && (
          <Pressable
            style={({pressed}) => [
              styles.goAheadButton,
              pressed && styles.buttonPressed,
            ]}
            onPress={onAllPermissionsGranted}>
            {({pressed}) => (
              <>
                <Text style={[styles.goAheadText, pressed && styles.textPressed]}>
                  Go Ahead
                </Text>
